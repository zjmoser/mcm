...
class YahooFinance {
    ...
    public function YahooFinance ($ticker='^GSPC') {
        $this->_ticker = $ticker;

        $this->_fromDate = array('d'=>date('j'), 'm'=>((int)date('m'))-1, 'y'=>((int)date('Y'))-1 );
        $this->_toDate   = array('d'=>date('j'), 'm'=>((int)date('m'))-1, 'y'=>date('Y'));
    }
    ...
    /**
     * Similar to getData() however only returns date and adjusted close.
     */
    public function getCloseData() {
        ...
        foreach ($lines as $line) {
            $elmts = explode(',', $line);
            $data[] = array(reset($elmts), (float)end($elmts));
        }
        return $data;
    }
